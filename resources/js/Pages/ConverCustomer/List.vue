...
import Filter from '@/Pages/ConverCustomer/Filter.vue';//検索
...
const savedFilters = sessionStorage.getItem('convercustomerFilters');
if (savedFilters) {
...
const handleSearch = (params) => {
  ...
  sessionStorage.setItem('convercustomerFilters', JSON.stringify(params));
...
const goToDetail = (customerCode) => {
  router.push({ path: '/convercustomer/detail', query: { key: customerCode } });
};
            ...
            <table class="table_w tablesorter alter" id="table_sort"> 
              ...
              <tbody>
                <tr v-for="(item, index) in items" :key="index">
                  <th class="item_f white-space">{{ item.得意先CD }}</th>
                  <td class="white-space">{{ item.得意先略名 }}</td>
                  <td class="sp_btn">
                    <button @click.prevent="goToDetail(item.M得意先_ID)" type="button" class="bo_btn">編集</button>
                  </td>
                </tr>
              </tbody>
            </table>
