  ...
  <div class="modal_wrap" v-if="isVisible" id="delete_modal">
    <div class="modal_inner s">
      ...
      <div class="signup_form">
        <h5>パスワードのロックを解除します。<br>
          よろしいですか？</h5>
      </div>
      <div class="btn_space_modal">
        <div class="submit_btn_no close_icon" @click="close">キャンセル</div>
        <div class="submit_btn_yes id_btn_yes" @click="rockstatuschenge">はい</div>
      </div>          
    </div>
  </div>
</template>
...
const selectedStaffCD = ref(null);
...
// パスワードを初期化する処理
const rockstatuschenge = async () => {
  try {
    const response = await axios.post('/api/staff/rockstatuschenge', {
      staffCD: selectedStaffCD.value,
    });
    if (response.data.success) {
      ElNotification({
          title: 'Success',
          message: 'パスワードのロックが解除されました。',
          type: 'success',
      });
    setTimeout(() => {
