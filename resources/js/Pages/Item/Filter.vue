...
const savedFilters = JSON.parse(sessionStorage.getItem('itemFilters') || '{}');
...
// Update filters to uppercase when filterName changes
watch(filterName, (newVal) => {
  filterName.value = newVal.toUpperCase();
  saveFilters();
});

// Watch other fields and save changes
watch([filterText, filtrItemName, yobikei1, yobikei2, yobikei3, radioOption], saveFilters);
...
// Toggle icon based on details state
