...
const addDispatch = async () => {
  try {
    ...
    const response = await axios.put('/api/dispatch-unavailable/add', {
      date: selectedDate.value // YYYYMMDD形式の日付
    });
    ...
    let errorMessage = '配車不可の設定に失敗しました。';
...
<div class="modal_wrap" id="delete_modal" v-if="isVisible">
  <div class="modal_inner s">
    ...
    <div class="signup_form">
      <h5>選択した日を配車不可にします。<br>
        よろしいですか？</h5>
    </div>
      <div class="btn_space_modal">
        <div class="submit_btn_no close_icon" @click="close">いいえ</div>
        <div class="submit_btn_yes id_btn_yes" @click.prevent="addDispatch">はい</div>
      </div>          
  </div>
</div>  
</template>
