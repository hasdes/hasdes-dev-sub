...
const mKey = route.query.key;//M商品_ID
const dKey = route.query.d_key;//HD変換商品_ID
...
const reLoadItem = () => {
  ...
  if (dKey) {
    axios.get('/api/converproduct/detail', {
...
const changeproduct = () => {
  ...
  if (!item.value.変換名 || item.value.変換名.trim() === '') {
    ...
    axios.put('/api/converproduct/edit', {
      'HD変換商品_ID': dKey,
      '商品CD': item.value.商品CD,
      '変換名': item.value.変換名
    }).then((res) => {
        ElNotification({
            title: 'Success',
  ...
  <section class="section dashboard">
    ...
    <div class="row">
      <ol class="breadcrumb">
        ...
        <li><router-link :to="{ path: '/converproduct/detail', query: { key: mKey } }">商品変換</router-link></li>
        <li>商品変換</li>
        <li>商品変更</li>
      </ol>
            ...
            <form v-if="item">
              <div class="row space align-center justify-content-between page group contents space_d">                  
                ...
                <div class="col-sp-12 btn_center ma_top_a line_up center">
                  ...
                  <button type="submit" @click.prevent="changeproduct" class="button_r none search od_a to">保存</button>        
                </div>
              </div>              
            </form>           
        </div>
