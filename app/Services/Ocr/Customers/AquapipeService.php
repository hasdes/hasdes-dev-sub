...
//　アクアパイプテック
//========================
class AquapipeService
{
    // ------------------
    // 注文情報抽出
    // ------------------

    public static function AquapipeOrderInfo(string $text, Document $document): array
    {
        ...
        try {
            ...
            // 1. 発注番号（ハイフン含む）
            if (preg_match('/注文番号:\s*([A-Z0-9-]+)/u', $text, $m)) {
                // $log->info('$m:', $m);
                $orderNumber = trim(mb_convert_kana($m[1] ?? '', 'a'));
            }
            // 2. 得意先
            $customerName = 'アクアパイプテック株式会社';

            // 3. 出荷先（納入先）
            if (preg_match('/(?:納入先|齁入先|入先)\s*[:：]?\s*([\s\S]*?)\s*TEL/u', $text, $m1)) {
                ...
                // $log->info('$raw3:'.$raw);
                if ($raw !== '') {
                    if (mb_strpos($raw, '倉庫') !== false) {
                        $shippingName = '倉庫物流センター';
                    }
                    elseif (mb_strpos($raw, '直送') !== false) {
                        $shippingName = $raw;
                    }
                    else {
                        $shippingName = $raw;
                        ...
                        try {
                            ...
                            foreach ($cells as $i => $cell) {
                                ...
                                if ($colDue !== null && $i === $colDue) {
                                    ...
                                    // 2026. 2. 9 / 2026-2-9 / 2026/2/9 など（区切り前後の空白を許可）
                                    if (preg_match(
                                        '/\b(\d{4})\s*[.\-\/]\s*(\d{1,2})\s*[.\-\/]\s*(\d{1,2})\b/u',
                                        $raw,
                                        $m
                                    )) {
                                    ...
                                    // 2026年 2月 9日（既にOKだけど一応そのまま）
                                    if (preg_match(
                                        '/\b(\d{4})年\s*(\d{1,2})月\s*(\d{1,2})日?\b/u',
                                        $raw,
                                        $m
                                    )) {
                                        $ymd = sprintf('%04d%02d%02d', $m[1], $m[2], $m[3]);
                                        $candidateDates[] = $ymd;
                                        // $log->info('$candidateDates2:'.$candidateDates);
                                        continue;
                                    }
                                }
            ...
            // ======================================
            if (empty($dueDate)) {
                if (preg_match('/\b(\d{4})\s*\.\s*(\d{1,2})\s*\.\s*(\d{1,2})\b/u', $text, $m)) {
                    $y = $m[1];
            ...
            //最終的に納期が空の場合、「最短納期」にする
            if (empty($dueDate)) {
                $shipDate = $dueDate = '最短納期';
            }
            ...
            // エラー内容をログ出力して終了
            $log->error('AquapipeOrderInfo 強制終了: ' . $e->getMessage());
    ...
    public static function AquapipeOrderItems(string $text, Document $document): array
    {
        ...
        try {
                        ...
                        foreach ($headerTexts as $i => $label) {
                            ...
                            foreach ($lines as $line) {
                                ...
                                if ($line === '') continue;// 空行をスキップ
                                    if (preg_match('/商品仕様|商品|仕様/u', $line)) $nameCol = $i;
                                    if (preg_match('/数量/u', $line)) $qtyCol = $i; //「実数」（数量列）を見つけたら、その列番号を $qtyCol に保存。
                            }
                        }
                        ...
                        foreach ($bodyRows as $rowIndex => $row) {
                            ...
                            try {
                                ...
                                if ($nameCol !== null && $nameCol < count($cells)) {
                                    ...
                                    $productText = DocumentAiHelperService::normalizeProductName($name, true);//商品名を整形
                                    
                                    // $log->info('$productText:'. $productText);//確認用
                                }

                                // ※ 上記で作った productText だけを使って処理
                                ...
                                // ▼ フィルタ：非商品行スキップ
                                if (
                                    preg_match('/(納入|記入|下さい|お願い)/u', $productName)
                                ) {
            ...
            $log->error('extractDefaultAquapipeItems 強制終了: ' . $e->getMessage());
            $log->error($e->getTraceAsString());
            throw $e;
        }
    }
